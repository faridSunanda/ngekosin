...
@section('content')
    <div class="max-w-4xl mx-auto">
        ...
        <div class="bg-white rounded-md border border-slate-200 p-6 shadow-xs">
            <form action="{{ route('admin.kos.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                @csrf
                ...
                <div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div class="md:col-span-2">
                            ...
                            <input type="text" name="name" value="{{ old('name') }}" placeholder="Contoh: Kos Griya Executive Tembalang" required
                        ...
                        <div class="md:col-span-2 bg-amber-50/50 p-4 rounded-md border border-amber-200/80 space-y-4">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                                <div>
                                    ...
                                    <div class="relative">
                                        ...
                                        <input type="number" name="price_per_month" value="{{ old('price_per_month', 1500000) }}" placeholder="1500000" required
                                ...
                                <div>
                                    ...
                                    <div class="relative">
                                        <span class="absolute left-3 top-2 text-xs font-bold text-slate-400">Rp</span>
                                        <input type="number" name="price_per_day" value="{{ old('price_per_day') }}" placeholder="Contoh: 150000"
                                               class="w-full pl-9 pr-3 py-2 bg-white border border-slate-200 rounded-md text-xs focus:outline-none focus:border-[#f99d18] text-[#20344c]">
                                    </div>
                                ...
                                <div>
                                    ...
                                    <div class="relative">
                                        <span class="absolute left-3 top-2 text-xs font-bold text-slate-400">Rp</span>
                                        <input type="number" name="price_per_week" value="{{ old('price_per_week') }}" placeholder="Contoh: 600000"
                                               class="w-full pl-9 pr-3 py-2 bg-white border border-slate-200 rounded-md text-xs focus:outline-none focus:border-[#f99d18] text-[#20344c]">
                                    </div>
                                    <span class="text-[10px] text-slate-400 mt-0.5 block">Kosongkan jika tidak ada</span>
                                </div>
                            ...
                            <div class="pt-3 border-t border-amber-200/60">
                                <label class="flex items-center gap-2 cursor-pointer">
                                    <input type="checkbox" name="allow_two_people" id="allowTwoPeople" value="1" {{ old('allow_two_people') ? 'checked' : '' }} onchange="toggleTwoPeoplePrice()"
                                           class="rounded text-[#f99d18] focus:ring-0">
                                    <span class="text-xs font-bold text-[#20344c]">Boleh diisi 2 Orang dalam 1 Kamar?</span>
                                </label>

                                <div id="twoPeopleContainer" class="mt-3 {{ old('allow_two_people') ? '' : 'hidden' }}">
                                    <label class="block text-xs font-bold text-[#20344c] mb-1">Harga Sewa / Bulan untuk 2 Orang (Rp)</label>
                                    <div class="relative max-w-sm">
                                        <span class="absolute left-3 top-2 text-xs font-bold text-slate-400">Rp</span>
                                        <input type="number" name="price_2_persons" value="{{ old('price_2_persons') }}" placeholder="Contoh: 2000000"
                                               class="w-full pl-9 pr-3 py-2 bg-white border border-slate-200 rounded-md text-xs focus:outline-none focus:border-[#f99d18] text-[#20344c]">
                                    </div>
                ...
                <div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <div class="md:col-span-3">
                            ...
                            <textarea name="address" rows="2" placeholder="Jl. Prof Soedarto No. 12, RT 02 / RW 05..." required
                                      class="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-md text-xs focus:outline-none focus:border-[#f99d18] text-[#20344c]">{{ old('address') }}</textarea>
                        </div>

                        <div class="md:col-span-3">
                            <label class="block text-xs font-bold text-[#20344c] mb-1">Link Google Maps (URL)</label>
                            <input type="url" name="google_maps_url" value="{{ old('google_maps_url') }}" placeholder="https://maps.app.goo.gl/xxx atau https://goo.gl/maps/xxx"
                                   class="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-md text-xs focus:outline-none focus:border-[#f99d18] text-[#20344c]">
                        ...
                        <div>
                            ...
                            <input type="hidden" name="province" id="provinceInput" value="{{ old('province', 'Jawa Tengah') }}">
                        </div>
                        ...
                        <div>
                            ...
                            <input type="hidden" name="city" id="cityInput" value="{{ old('city', 'Kota Semarang') }}">
                        </div>
                        ...
                        <div>
                            ...
                            <input type="hidden" name="district" id="districtInput" value="{{ old('district', 'Tembalang') }}">
                        </div>
                        ...
                        <div>
                            ...
                            <input type="hidden" name="village" id="villageInput" value="{{ old('village', 'Bangsri') }}">
                        </div>

                        <div>
                            <label class="block text-xs font-bold text-[#20344c] mb-1">Total Kamar <span class="text-red-500">*</span></label>
                            <input type="number" name="total_rooms" value="{{ old('total_rooms', 10) }}" required min="1"
                                   class="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-md text-xs focus:outline-none focus:border-[#f99d18] text-[#20344c]">
                        </div>

                        <div>
                            <label class="block text-xs font-bold text-[#20344c] mb-1">Kamar Tersedia <span class="text-red-500">*</span></label>
                            <input type="number" name="available_rooms" value="{{ old('available_rooms', 3) }}" required min="0"
                                   class="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-md text-xs focus:outline-none focus:border-[#f99d18] text-[#20344c]">
                        </div>
                    </div>
                </div>
                ...
                <div>
                    ...
                    <div id="campusContainer" class="space-y-3">
                        <!-- Dynamic campus row will be appended here -->
                    </div>
                </div>
                    ...
                    <div class="space-y-4">
                            ...
                            <div class="grid grid-cols-2 sm:grid-cols-3 gap-2 text-xs text-slate-700">
                                @forelse ($facilities as $fac)
                                    <label class="flex items-center gap-2 p-2 rounded-md bg-slate-50 border border-slate-200 cursor-pointer hover:border-[#f99d18] transition">
                                        <input type="checkbox" name="facilities[]" value="{{ $fac->name }}" {{ in_array($fac->name, old('facilities', [])) ? 'checked' : '' }} class="rounded text-[#f99d18] focus:ring-0">
                                        <span>{{ $fac->name }}</span>
                                    </label>
                                @empty
                        ...
                        <div>
                            <label class="block text-xs font-bold text-[#20344c] mb-1">
                                ...
                                <span class="text-slate-400 font-normal ml-1">(Maksimal 5 MB, Format: JPG, PNG, WEBP)</span>
                            </label>
                            ...
                            <div class="flex items-start gap-4">
                                ...
                                <div id="previewContainer" class="w-24 h-24 rounded-md border border-slate-200 bg-slate-50 overflow-hidden hidden shrink-0 relative">
                                    <img id="thumbnailPreview" src="#" alt="Pratinjau Foto" class="w-full h-full object-cover">
                                </div>
                            </div>
                        </div>

                        <!-- Gallery Images Upload (Max 6, 5MB each) -->
                        <div class="md:col-span-2">
                            <label class="block text-xs font-bold text-[#20344c] mb-1">Galeri Foto Tambahan Kos (Maksimal 6 Foto)</label>
                            <span class="text-[10px] text-slate-500 mb-2 block">Upload hingga 6 foto galeri pendukung (fasilitas kamar, kamar mandi, area bersama, dll). Maksimal 5MB per file.</span>
                        ...
                        <div>
                            ...
                            <textarea name="description" rows="4" placeholder="Jelaskan keunggulan kos, akses transportasi, lingkungan..."
                                      class="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-md text-xs focus:outline-none focus:border-[#f99d18] text-[#20344c]">{{ old('description') }}</textarea>
                        </div>
                        ...
                        <div>
                            ...
                            <select name="status" required class="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-md text-xs focus:outline-none focus:border-[#f99d18] text-[#20344c]">
                                <option value="active" {{ old('status') == 'active' ? 'selected' : '' }}>Aktif (Publikasikan Langsung)</option>
                                <option value="pending" {{ old('status') == 'pending' ? 'selected' : '' }}>Pending Verifikasi</option>
    ...
    <script>
        ...
        async function initRegionCascade(initialProvName = 'Jawa Tengah', initialCityName = 'Kota Semarang', initialDistName = 'Tembalang', initialVillageName = '') {
            try {
                ...
                provinces.forEach(p => {
                    ...
                    if (p.name.toLowerCase() === initialProvName.toLowerCase()) {
                ...
                provSelect.innerHTML = '<option value="">Jawa Tengah</option>';
                citySelect.innerHTML = '<option value="">Kota Semarang</option>';
                distSelect.innerHTML = '<option value="">Tembalang</option>';
                villageSelect.innerHTML = '<option value="">Tembalang</option>';
            }
        }
        ...
        document.addEventListener('DOMContentLoaded', () => {
            initRegionCascade("{{ old('province', 'JAWA TENGAH') }}", "{{ old('city', 'KOTA SEMARANG') }}", "{{ old('district', 'TEMBALANG') }}", "{{ old('village', 'TEMBALANG') }}");
            addCampusRow();
        });
    </script>
@endsection
